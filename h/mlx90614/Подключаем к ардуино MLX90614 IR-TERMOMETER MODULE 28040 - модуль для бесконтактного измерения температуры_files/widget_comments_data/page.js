var Page = {
  ...
  toggleSubscription: function(btn, hash, ev, oid, source, onDone, disableProgress) {
    ...
    ajax.post('al_wall.php', {act: 'a_toggle_posts_subscription', subscribe: act ? 1 : 0, oid: oid ? oid : cur.oid, hash: hash, source: source}, options);
    cancelEvent(ev);
  },
  ...
  togglePodcastsSubscription: function(btn, hash, act, ev, onDone, disableProgress) {
    ...
    ajax.post('al_wall.php', {act: 'a_toggle_podcasts_subscription', subscribe: act ? 1 : 0, owner_id: cur.oid, hash: hash}, {
    onDone: function(text) {
  ...
  showMessagesFromCommunityTooltip: function (btn, baseState, text, red) {
    if (cur.toggleMessagesFromCommunityAct == undefined && baseState == 0) {
      if (red) {
        showTooltip(btn, {text: text, dir: 'auto', shift: [4, -3]});
      } else {
  ...
  getPostModuleCode: function(module) {
    switch(module) {
      ...
      case 'feed_friends': return 'rr';
      case 'feed_friends_recent': return 'rr';
      case 'feed_friends_top': return 'tr';
      case 'feed_groups': return 'rg';
      case 'feed_groups_recent': return 'rg';
      case 'feed_groups_top': return 'tg';
      case 'feed_videos': return 'rv';
      case 'feed_videos_recent': return 'rv';
      case 'feed_videos_top': return 'tv';
  ...
  postsSend: function() {
    ...
    var query_data = {act: 'seen', data: data.join(';'), meta: meta.join(';')};
    if (window._postsViewHash) {
      query_data['hash'] = window._postsViewHash;
    }
    ajax.post('al_page.php', query_data, {onDone: function() {
      if (!ls.checkVersion()) {
  ...
  infoSave: function() {
    ...
    }, showProgress: lockButton.pbind('currinfo_save'), hideProgress: unlockButton.pbind('currinfo_save'), stat: ['tooltips.js', 'tooltips.css', jsc('web/emoji.js')]});
  },
      ...
      if (canPlay) {
        ...
        showBox('docs.php', {
          ...
          onDone: function(box, data) {
            if (!vk.widget) {
              curBox().setOptions({width: data.width + 50 /* box lr padding sum */})
  ...
  showGif: function(linkEl, ev, dontHideActive, canPlayMp4) {
    ...
    var progressIconHTML = '<div class="page_gif_progress_icon" style="display:none;">' + rs(vk.pr_tpl, {id: '', cls: ''}) + '</div>';
  ...
  initVideoAutoplay: function(canPlayMp4) {
    ...
    if (cur.module == 'feed') {
      ...
      var chatHeader = geByClass1('im-page--chat-header'), chatHeaderHeight = getSize(chatHeader)[1];
    ...
    switch (action['type']) {
      case BTN_TYPE_OPEN_URL:
        if (action['is_internal'] && action['open_in_layer']) {
          ...
          return showWiki({w: page}, null, null, {ads_params: params});
        }
...
var Wall = {
    ...
    ajax.post('al_wall.php', {
      ...
      onDone: function(text) {
        ...
        if (!deleteBlock) {
          ...
          hide(postContent);
          deleteBlock = ce('div', { id: 'post_del' + post, className: 'dld', innerHTML: '<span class="dld_inner">' + text + '</span>' });
        ...
        if (actionsWrap) {
          ...
          if (!progressElement) {
            progressElement = ce('div', { className: 'progress' });
  ...
  restoreThread: function(el, post, hash) {
    ajax.post('al_wall.php', { act: 'restore_thread', post: post, hash: hash }, {
      onDone: function() {
  ...
  receive: function(rows, names, type) {
    ...
    var isTop = (type === 'top');
    var n = ce('div', {innerHTML: rows}), revert = !!cur.options.revert;
  ...
  showMore: function(offset, tabEl) {
    ...
    ajax.post('al_wall.php', {act: 'get_wall', owner_id: cur.oid, offset: offset, type: type, fixed: cur.options.fixed_post_id || '', wall_start_from: wallNextFrom, onlyCache: onlyCache}, {
  ...
  getOwnerDraft: function(ownerId) {
    ...
    return [draft.txt, draft.medias, true, {from: intval(draft.from), signed: intval(draft.signed), shareShowImg: intval(draft.shareShowImg)}, draft.bkg_id];
  },
  ...
  getDraftData: function() {
    ...
    var addmedia = cur.wallAddMedia || {},
        ...
        params = {message: msg, bkg_id: bkg_id};
  ...
  checkAsGroup: function(el) {
    checkbox(el);
    wall.setReplyAsGroup(el, { fromGroup: isChecked(el) });
  },
  ...
  repliesSideUpdate: function (st) {
    ...
    if (st < postData[2]) {
      ...
      newStyle = {marginTop: postData[3] - postData[2], opacity: 0, display: null};
    }
  ...
  moreReplies: function(post, offset, count, opts) {
    ...
    if (opts.deep) {
      ...
      params = {act: 'get_post_replies', offset: offset, owner_id: postItem[0], item_id: postItem[1], count: count};
    } else {
      params = {act: 'get_replies', offset: offset, post: post, count: count};
    }
  ...
  showNextReplies: function(showMore, itemFullId, event) {
    ...
    var loader = ce('span', { className: 'progress_inline replies_next_loader' });

    ajax.post('al_wall.php', params, {
      onDone: function(replies, names, data) {
        var repliesList = domChildren(ce('div', { innerHTML: replies }));
        var fragment = cf();
        var hasNew = false;
  ...
  ignoreAdsItem: function(post_raw, feed_raw, hash) {
    ...
    ajax.post('/al_feed.php?misc', {act: 'a_ignore_item', post_raw: post_raw, feed_raw: feed_raw, hash: hash, ad_data: adData, no_html: 1}, {
      onDone: function() {
        hide(postEl);
      }
    });
  ...
  ignoreAdsOwner: function(post_raw, owner_id, hash, btn) {
    ...
    ajax.post('/al_feed.php?misc', {act: 'a_ignore_owner', post_raw: post_raw, owner_id: owner_id, hash: hash, list: 0, no_html: 1}, {
  ...
  triggerAdStat: function(elIdOrEl, event) {
    ...
    while (pixels) {
      ...
      if (adBlockUID && isUniqueEvent) {
        try {
          ...
          try {console.log(e.message);} catch(e2) {}
        }
      }
    ...
    if (statHtml) {
      var statElemWrap = ce('div', {innerHTML: statHtml});
      var elem;
        ...
        onDone: function(text, confirmText, isClosed) {
          if (confirmText) {
            var box = showFastBox(getLang('global_warning'), confirmText, getLang('group_leave_group'), function() {
              box.hide();
              Wall.subscribeToAuthor(subscribed, authorId, hash, reqOptions, '', { confirm: 1 });
            }, getLang('global_cancel'));
            return true;
          }
  ...
  getNewPostHTML: function(ev, adminLevel, extendCb, cur) {
    ...
    repls = {
      ...
      reply_box: ev[8] == 1 ? rs(cur.wallTpl.reply_box, { actions: '', class: canReplyAsGroup ? ' reply_fakebox_with_official' : '' }) : '',
      photo: psr(thumbs[0]),
      link: ev[5],
      text: psr(ev[6]),
      date: date,
    ...
    if (isEditor && cur.wallTpl.author_data) {
      repls.date = rs(cur.wallTpl.author_data, {date: repls.date, post_id: post_id});
    }
  ...
  replyAsGroupTT: function(obj, owner) {
    ...
    if (!ttChooser) {
      var list = wall.replyAsGroupList(obj, owner), rows = '';
      ...
      ttChooser = new ElementTooltip(obj, {
        ...
        onFirstTimeShow: function(ttel) {
          ...
          each (rowEls, function(i, row) {
            ...
            addEvent(row, 'click', wall.setReplyAsGroup.pbind(obj, {from: domData(this, 'from-oid')}));
          });
                  ...
                  ajax.plainpost(searchUrl, {
                    ...
                    each(response, function () {
                      if (!ge('_post_from_tt_row_oid_'+this[0])) {
                        ...
                        addEvent(newRow, 'click', wall.setReplyAsGroup.pbind(obj, {from: domData(newRow, 'from-oid')}));
    ...
    if (owner && ttChooser.isShown() && !obj.hasAttribute('data-disallow-profile')) {
      wall.setReplyAsGroup(obj, {from: (from == vk.id ? owner : vk.id)});
    }
  ...
  viewsUpdate: function(el, post_id) {
    ...
    ajax.post('/like.php', params, {
      ...
      onDone: function(cnt, title) {
        ...
        var fakeEl = ce('div', {innerHTML: title});
  ...
  toggleRecommFriend: function(btn, ev, hash) {
    ...
    var params = { act: 'add', mid: mid, hash: hash, from: from, ts: vkNow() };
  ...
  hideRecommFriend: function(btn, ev, hash) {
    ...
    ajax.post('al_friends.php', { act: 'hide_possible', mid: mid, hash: hash, from: from }, {
      onDone: function() {
  ...
  _friendsRecommLogSend: function() {
    ...
    if (logs.length) {
      ...
      ajax.post('al_friends.php', { act: 'a_recomm_logs', logs: logs }, {
        onDone: function() {
  ...
  uploadFailed: function(info, code) {
    ...
    if (Upload.types[i] == 'fileApi' && !options.wiki_editor) {
      ...
      if (options.filesTotalCount == 1) {
        setTimeout(showFastBox({title: getLang('global_error')}, getLang('wall_add_photo_error')).hide, 2000);
      }
    }
    // hide(box.progress);
...
Composer = {
  ...
  onItemSelect: function (composer, item) {
    ...
    // В сообщениях не вставляем алиасы по умолчанию
